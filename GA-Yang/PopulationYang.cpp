...
CPopulationYang::CPopulationYang(default_random_engine& randomEngine, CGenotypeData& genotypeData, unsigned int numberOfSNPs) : 
    CPopulation(randomEngine, genotypeData, numberOfSNPs, CChromosomeYang::NUMBER_OF_GENOTYPES), mCurrentGeneration(NULL), mNextGeneration(NULL)
{
}

CPopulationYang::~CPopulationYang()
{
...
void CPopulationYang::Init()
{   
    mCurrentGeneration = new CChromosomeYang[mSize];
    mNextGeneration = new CChromosomeYang[mSize];

    // Build chromosomes
    for (unsigned int i=0; i<mSize; i++) 
    {
        //mNextGeneration[i] = new CChromosomeYang();
        //mCurrentGeneration[i] = new CChromosomeYang();
...
// bool ComparePred( CChromosomeYang elem1, CChromosomeYang elem2 )
// {
//     return elem1 < elem2;
// }

void CPopulationYang::Fiteness()
{
    // Calculate fitness 
    mFitnessMax = FLT_MIN;
    mFitnessMin = FLT_MAX;
...
void CPopulationYang::NextGeneration()
{
...
bool CPopulationYang::CheckForDuplication(const CChromosomeYang& input, unsigned int currentIndex)
{
    // If it has already been created then drop it (by decreasing the counter)
...
void CPopulationYang::DifferentChromosomes(vector<const CChromosome*>& oChromosomes, int iNumberOfSolutions) const 
{
...
unsigned int CPopulationYang::Elitism()
{
    ...
    {
        mNextGeneration[nextGenIndex++] = *dynamic_cast<const CChromosomeYang*>(differentChromosomes[i]);
    }
...
void CPopulationYang::Vibrate(double lastBestFitness)
{
